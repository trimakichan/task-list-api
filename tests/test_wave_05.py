...
# @pytest.mark.skip(reason="test to be completed by student")
def test_get_goal_invalid_id(client):
    pass
    # Act
    ...
    assert response_body == {"message" : "Goal (goal) is invalid."}
...
def test_update_goal_not_found(client):
    ...
    assert response.status_code == 404
    assert response_body == {"message" : "Goal (1) is not found."}
    assert db.session.scalar(db.select(Goal).where(Goal.id == 1)) is None


def test_update_goal_invalid_id(client, one_goal):
    # Act
    response = client.put("/goals/goal", json={
        "title": "Updated title",
    })
    response_body = response.get_json()
